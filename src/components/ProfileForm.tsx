...
export default function ProfileForm({ user }: ProfileFormProps) {
  ...
  const [profilePic, setProfilePic] = useState<UploadResponse | undefined>(
    undefined
  );
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    const hasInvalidDates = certifications.some((cert) => {
      ...
      return (
        isNaN(issued.getTime()) || isNaN(expiry.getTime()) || issued >= expiry
      );
    });
  ...
  return (
    <form
      ...
      className="max-w-4xl mx-6 md:mx-auto grid grid-cols-1 lg:grid-cols-2 lg:gap-12 flex-wrap"
    >
