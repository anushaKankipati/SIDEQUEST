...
export default function MyFavoritesPage({ initialFavorites, userEmail }: { initialFavorites: any[]; userEmail: string }) {
  ...
  const handleFavoriteChange = async (adId: string) => {
    try {
      ...
      setFavoriteAds((prevFavorites) =>
        prevFavorites.filter((ad) => ad.id !== adId) // Remove the ad from favorites
      );
  ...
  return (
    <div className="container mb-6 my-12 mx-auto">
      <h1 className="text-2xl font-bold mb-6 text-center pt-10">Your Favorite Quests</h1>
