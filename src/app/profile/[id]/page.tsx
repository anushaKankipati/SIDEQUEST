          ...
          <div className="relative">
            <div className="flex items-start space-x-6">
              ...
              <div>
                ...
                <p className="text-gray-500 text-s mt-4">Joined: {formatDate(user.createdAt)}</p>
              </div>
            </div>
          ...
          {user.Quest.map((quest: any) => (
            <Link href={`/ad/${quest.id}`} key={quest.id}>
              <div className="p-4 bg-white rounded-lg shadow-sm hover:shadow-md transition-shadow mb-4"> 
                <h3 className="font-semibold text-theme-green">{quest.title}</h3>
                <div className="flex justify-between mt-2 text-sm text-gray-600">
                    <span>{quest.category === "hourly" ? `$${quest.price}/hr` : `$${quest.price} (fixed)`}</span>
